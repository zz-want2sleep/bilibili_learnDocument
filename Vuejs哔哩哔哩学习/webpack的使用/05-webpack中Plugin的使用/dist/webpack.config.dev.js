 ...
 * @LastEditTime: 2020-08-26 07:03:34
...
var HtmlWebpackPlugin = require('html-webpack-plugin'); //将自动生成一个index.html文件（可以自定义模板），会将打包完后的js自动使用script标签插入body中


var UglifyJsPlugin = require('uglifyjs-webpack-plugin'); //用来压缩打包完后的js文件（丑化）
...
module.exports = {
  ...
  output: {
    ...
    filename: 'bundle.js' // 加上publicPath后涉及到url的东西，打包后路径都会加上dist/
  ...
  plugins: [new webpack.BannerPlugin('最终版权归zz所有'), // 安装插件（用于扩展框架）
